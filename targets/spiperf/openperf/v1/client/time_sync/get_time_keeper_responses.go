// Code generated by go-swagger; DO NOT EDIT.

package time_sync

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/Spirent/openperf/targets/spiperf/openperf/v1/models"
)

// GetTimeKeeperReader is a Reader for the GetTimeKeeper structure.
type GetTimeKeeperReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetTimeKeeperReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewGetTimeKeeperOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("unknown error", response, response.Code())
	}
}

// NewGetTimeKeeperOK creates a GetTimeKeeperOK with default headers values
func NewGetTimeKeeperOK() *GetTimeKeeperOK {
	return &GetTimeKeeperOK{}
}

/*GetTimeKeeperOK handles this case with default header values.

Success
*/
type GetTimeKeeperOK struct {
	Payload *models.TimeKeeper
}

func (o *GetTimeKeeperOK) Error() string {
	return fmt.Sprintf("[GET /time-keeper][%d] getTimeKeeperOK  %+v", 200, o.Payload)
}

func (o *GetTimeKeeperOK) GetPayload() *models.TimeKeeper {
	return o.Payload
}

func (o *GetTimeKeeperOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.TimeKeeper)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
