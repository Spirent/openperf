...
TEST_CASE("uuid constructors", "[uuid]")
{
    ...
    {
        uuid u;
        for (size_t i = 0; i < 15; i++) { REQUIRE(u[i] == 0); }
    ...
    SECTION("can construct UUID from string")
    {
        ...
        SECTION("can convert to string correctly")
        {
            REQUIRE(openperf::core::to_string(u)
        ...
        SECTION("can reject obviously bad strings")
        {
            ...
            REQUIRE_THROWS(
                new uuid("123e4567-e89b-12d3-a456-4266554400")); /* too short */
            REQUIRE_THROWS(new uuid(
    ...
    SECTION("can construct UUID from bytes")
    {
        ...
        {
            auto p = u.data();
            for (size_t i = 0; i < 15; i++) { REQUIRE(p[i] == data[i]); }
        }
