...
using raw_allocator =
    openperf::memory::std_allocator<uint8_t,
...
TEST_CASE("basic functionality for std_allocator", "[std_allocator]")
{
    ...
    SECTION("can create, ")
    {
        ...
        {
            auto mempool = raw_allocator(
                reinterpret_cast<uintptr_t>(raw_pool.get()), pool_size);
            ...
            SECTION("can deallocate, ") { mempool.deallocate(a1, 1024); }
        ...
        SECTION("get exception when out of memory, ")
        {
            ...
            for (size_t allocated = 0; allocated < (pool_size - alloc_size);
            ...
            REQUIRE_THROWS_AS(mempool.allocate(alloc_size), std::bad_alloc);

            /* Release all memory */
            for (auto p : ptrs) { mempool.deallocate(p, alloc_size); }
        }
...
using int_allocator =
    openperf::memory::std_allocator<int,
                                    openperf::memory::allocator::free_list>;
...
TEST_CASE("able to use std_allocator in a container", "[std_allocator]")
{
    ...
    REQUIRE(raw_pool);
    auto int_pool =
        int_allocator(reinterpret_cast<uintptr_t>(raw_pool.get()), pool_size);

    SECTION("can handle container allocations, ")
    {
        std::vector<int, int_allocator> test_vec(int_pool);
        for (int i = 1; i < 100; i++) { test_vec.push_back(i); }
