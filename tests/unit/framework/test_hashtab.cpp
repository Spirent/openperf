...
TEST_CASE("op_hashtab functionality checks", "[hashtab]")
{
    SECTION("can create, ") {
        ...
        SECTION("can insert, ") {
            ...
            SECTION("can count, ") {
                REQUIRE(op_hashtab_size(tab) == 1);
            }

            SECTION("can delete, ") {
                bool deleted = op_hashtab_delete(tab, reinterpret_cast<void*>(key));
                REQUIRE(deleted);

                SECTION("cannot find deleted, ") {
                    auto not_here = op_hashtab_find(tab, reinterpret_cast<void*>(key));
...
struct op_hashtab_deleter {
    void operator()(op_hashtab *tab) const {
        op_hashtab_free(&tab);
    }
};
...
TEST_CASE("op_hashtab concurrent inserts work", "[hashtab]")
{
    ...
    SECTION("can create, ") {
        std::unique_ptr<op_hashtab, op_hashtab_deleter> tab(op_hashtab_allocate());
        REQUIRE(tab);
        ...
        SECTION("can insert with threads, ") {
            for (auto scaler : scalers) {
                threads.emplace_back(std::thread([&, scaler]() {
                                                     for (size_t i = 1; i <= nb_inserts; i++) {
                                                         ...
                                                         bool inserted = op_hashtab_insert(tab.get(),
                                                                                            reinterpret_cast<void*>(key),
            ...
            SECTION("can accurately count inserted items, ") {
                REQUIRE(op_hashtab_size(tab.get()) == nb_inserts * scalers.size());
            }
            ...
            SECTION("can find all inserted items, ") {
                for (auto scaler : scalers) {
                    for (size_t i = 1; i < nb_inserts; i++) {
                        uintptr_t key = scaler * i;
                        auto found = op_hashtab_find(tab.get(),
                                                      reinterpret_cast<void*>(key));
