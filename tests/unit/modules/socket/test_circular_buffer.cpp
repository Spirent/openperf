...
struct circular_buffer
{
    ...
    {}
    circular_buffer() {}
    ~circular_buffer() { delete[] base_ptr; }
};

class test_buffer_producer
    : public circular_buffer
...
TEST_CASE("circular buffer functionality", "[circular buffer]")
{
    ...
    /*
     * Yeah this is a little weird.  The consumer and producer are different
     * view of the same underlying structure.
     */
    ...
    SECTION("can query buffer state, ")
    {
        ...
        {
            static constexpr std::string_view test_string =
                "This is a silly test string.";
            auto written = producer->write(
            ...
            SECTION("can read written data, ")
            {
                ...
                REQUIRE(consumer->readable() == 0);
                REQUIRE(std::memcmp(buffer.data(), test_string.data(), read)
                        == 0);
            }
            ...
            SECTION("can read written data a byte at a time, ")
            {
                ...
                while (consumer->readable()) {
                    auto read =
                        consumer->read(char_buffer.data(), char_buffer.size());
                    REQUIRE(read == 1);
                    count++;
                }
