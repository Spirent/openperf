...
struct test_sink
{
    ...
    bool operator==(const test_sink& other) const
    {
        return (id == other.id);
    }
};
...
TEST_CASE("forwarding table functionality", "[forwarding table]")
{
    ...
    SECTION("insert many interfaces, ") {
        ...
        for(auto& [key, value] : interfaces) {
            table.insert_interface(key.first, key.second, std::addressof(value));
        }
        ...
        SECTION("find many interfaces, ") {
            ...
            SECTION("remove many interfaces, ") {
                for (const auto& [key, value] : interfaces) {
                    auto to_delete = table.remove_interface(key.first, key.second);
                    REQUIRE(to_delete);
                    delete to_delete;
                }
    ...
    SECTION("insert many sinks, ") {
        ...
        SECTION("find many sinks, ") {
            ...
            std::set_difference(std::begin(port0_sinks), std::end(port0_sinks),
                                std::begin(table_port0_sinks), std::end(table_port0_sinks),
            ...
            std::set_difference(std::begin(port1_sinks), std::end(port1_sinks),
                                std::begin(table_port1_sinks), std::end(table_port1_sinks),
                                std::back_inserter(difference),
                                [](const test_sink& a, const test_sink& b) {
                                    return (a.id < b.id);
                                });
