...
class _be_valid_network_tvlp_profile(Matcher):
    def _match(self, profile):
...
class _be_valid_tvlp_result(Matcher):
    def _match(self, result):
        ...
        for prof in [result.memory, result.block, result.cpu, result.network]:
...
be_valid_network_tvlp_profile = _be_valid_network_tvlp_profile()
