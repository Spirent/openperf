...
class DocumentationFormatter(Formatter):
    ...
    def _format_example(self, symbol, example):
        puts('  ' * self._depth(example) + symbol + ' ' +
             example.name + self._format_slow_test(example))
    ...
    def _format_example_group(self, example_group, color_name):
        puts('  ' * self._depth(example_group) +
             self._color(color_name, example_group.name))
    ...
    def failures(self, failed_examples):
        ...
        with indent(2):
            for index, failed in enumerate(failed_examples):
                puts('%d) %s' %
                     (index + 1, self.format_full_example_name(failed)))
                with indent(3):
    ...
    {exc_type}: {exc_msg}
        """.format(
            filename=filename,
            source_line=open(filename).read().splitlines()[
                tb.tb_lineno-1].strip(),
...
class ProgressFormatter(DocumentationFormatter):
    ...
    def summary(self, duration, example_count, failed_count, pending_count, example_skipped):
        puts()
        puts()
        super(ProgressFormatter, self).summary(
            duration, example_count, failed_count, pending_count, example_skipped)
