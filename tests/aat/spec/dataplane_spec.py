...
POLL_SECOND = 1000                    # poll timeout in ms
...
def ping_command(ping_binary, address):
    """Generate a ping shell command for Popen"""
    # One ping only...
    args = [ping_binary, '-c 1', '-n', address]
...
def wait_for_keyword(output_stream, keyword):
    ...
    while not found:
        events = poller.poll(5 * POLL_SECOND)
...
def create_connected_endpoints(api_client, reader_id, writer_id, domain, protocol, null):
    ...
    run the risk of the the client failing to connect before the server is started
    ...
    if not is_server_interface(reader_id):
        reader = subprocess.Popen(nc_command(server_ip_addr, protocol=protocol, verbose=True),
                                  ...
                                  env={'LD_PRELOAD': shim.config.path,
                                       'ICP_BINDTODEVICE': reader_id})
        output = reader.stderr

    try:
        wait_for_keyword(output, "succeeded")
    ...
    with open(os.devnull, 'w') as null:
        ...
        try:
            while total_read < io_size:
                events = poller.poll(POLL_SECOND)

                if not len(events):
                    raise RuntimeError("Timeout read={}, write={}".format(total_read, total_written))
...
with description(None, 'Dataplane') as self:
            ...
            with description('bulk data transfer,'):
                with description('client to server,'):
                    with it('succeeds,'):
                        ...
                        # read more packets that we intended to send, but that's ok.
