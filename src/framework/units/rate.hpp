 ...
 *
 * In addition to standard operations and casts as found in std::chrono::duration,
 * we also provide a helper function to generate a duration based on the
 * frequency.
 */
...
template <typename FromRate, typename ToRate,
          typename Frequency = typename std::ratio_divide<typename FromRate::frequency,
...
constexpr
typename std::enable_if_t<is_rate<ToRate>::value, ToRate>
...
template <typename FromRate, typename ToDuration,
          typename Frequency = typename std::ratio_multiply<typename FromRate::frequency,
...
constexpr
ToDuration get_period(const rate<Rep, Frequency>& from)
{
...
class rate
{
    ...
    static_assert(is_ratio<Frequency>::value, "Rate frequency must be a std::ratio");
...
public:
    ...
    template <typename Rep2,
              typename = typename std::enable_if_t<std::is_convertible<const Rep2&, rep>::value
    ...
    template <typename Rep2,
              typename Frequency2,
              typename = typename std::enable_if_t<treat_as_floating_point<rep>::value
...
/* Equality */
template <typename LeftRate, typename RightRate>
struct rate_equal_impl
{
...
struct rate_equal_impl<Rate, Rate>
{
...
template <typename Rep1, typename Frequency1,
          typename Rep2, typename Frequency2>
constexpr
bool operator==(const rate<Rep1, Frequency1>& left, const rate<Rep2, Frequency2>& right)
{
    return (rate_equal_impl<rate<Rep1, Frequency1>, rate<Rep2, Frequency2>>()(left, right));
}
...
template <typename Rate>
struct rate_less_than_impl<Rate, Rate>
{
    bool operator()(const Rate& left, const Rate& right) const
    {
        return (left.count() < right.count());
...
bool operator<(const rate<Rep1, Frequency1>& left, const rate<Rep2, Frequency2>& right)
{
    return (rate_less_than_impl<rate<Rep1, Frequency1>, rate<Rep2, Frequency2>>()(left, right));
}
...
operator+(const rate<Rep1,Frequency1>& left, const rate<Rep2,Frequency2>& right)
{
    using common = std::common_type_t<rate<Rep1, Frequency1>, rate<Rep2,Frequency2>>;
...
constexpr
typename std::enable_if_t<std::is_convertible<Rep2, std::common_type_t<Rep1, Rep2>>::value,
...
operator*(const rate<Rep1, Frequency>& value, const Rep2& scalar)
{
    ...
    return (common_rate(common_rate(value).count() * static_cast<common_rep>(scalar)));
}
