...
public:
    ...
    int run(int timeout) { return op_event_loop_run_timeout(m_loop.get(),
                                                             timeout); }
    ...
    int update(uint32_t timeout_id,
               const struct op_event_callbacks *callbacks)
    {
        return op_event_loop_update_timer_cb(m_loop.get(), timeout_id, callbacks);
    }
    int update(uint32_t timeout_id,
               uint64_t timeout)
    {
    ...
    int disable(int fd)
    {
        return op_event_loop_disable_fd(m_loop.get(), fd);
    }
    ...
    int del(int fd)
    {
        return op_event_loop_del_fd(m_loop.get(), fd);
    }
    int del(void *socket)
    {
