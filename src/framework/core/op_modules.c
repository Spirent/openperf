...
static SLIST_HEAD(op_modules_list, op_module) op_modules_list_head =
    SLIST_HEAD_INITIALIZER(op_modules_list_head);
...
void op_modules_register(struct op_module *module)
{
    ...
    if(regresult)
    {
        ...
        size_t error_string_len = regerror(regresult, &regex, error_string, op_modules_err_str_max_len);
        op_exit("Module registration id validation regex failed to compile: %s%s\n", error_string, error_string_len == op_modules_err_str_max_len ? "(truncated)" : "");
    }
...
int op_modules_get_info_list(const struct op_module_info * info[], size_t max_entries)
{
    ...
    if (max_entries == 0)
        return op_modules_get_loaded_count();
