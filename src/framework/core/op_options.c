...
static SLIST_HEAD(op_options_data_head, op_options_data) op_options_data_head =
    SLIST_HEAD_INITIALIZER(op_options_data_head);
...
static const struct op_option * _find_opt_by_long(const char * long_op, size_t len)
{
    ...
    SLIST_FOREACH(opt_data, &op_options_data_head, next) {
        for (const struct op_option *curr = opt_data->options;
            ...
            longopts[idx++] = (struct option){
                ...
                NULL,
                curr->short_opt ? curr->short_opt : op_options_hash_long(curr->long_opt)
            };
        }
    }
