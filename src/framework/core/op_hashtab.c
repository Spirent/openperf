...
static void* dummy_value =
    (void*)0x0ddba11; /**< Used to tag dummy key/value entries */
 ...
 */
static __attribute__((pure)) uintptr_t _hashtab_hash(const void* thing)
{
...
static __attribute__((pure)) int _hashtab_comparator(const void* thing1,
                                                     const void* thing2)
{
    const void* key1 =
        thing1 ? ((const struct op_hashtab_key_value*)thing1)->key : NULL;
    const void* key2 =
        thing2 ? ((const struct op_hashtab_key_value*)thing2)->key : NULL;
...
static const uint8_t _bit_reverse_table[256] = {R6(0), R6(2), R6(1), R6(3)};

static __attribute__((const)) uintptr_t _reverse_bits(uintptr_t x)
{
...
static __attribute__((const)) uintptr_t _get_dummy_key(uintptr_t key)
{
    return _reverse_bits(key & ~0x8000000000000000);
}

static __attribute__((const)) uintptr_t _get_regular_key(uintptr_t key)
{
    return _reverse_bits(key | 0x8000000000000000);
}

static __attribute__((const)) uintptr_t _get_parent(uintptr_t idx)
{
...
static void _hashtab_key_value_free(void* thing)
{
    ...
    if (kv->key_destructor) { kv->key_destructor(kv->key); }

    if (kv->value_destructor) { kv->value_destructor(kv->value); }
 ...
 * added to the table.
 */
static void _hashtab_item_free(struct op_list_item** itemp)
{
    free(op_list_item_data(*itemp));
    op_list_item_free(itemp);
}

static void _initialize_bucket(struct op_hashtab* tab, size_t bucket_idx)
{
    ...
    struct op_list_item* dummy = _hashtab_item_allocate(
        tab, (void*)_get_dummy_key(bucket_idx), dummy_value);
    assert(dummy);
...
static size_t _get_bucket_size(struct op_hashtab* tab)
{
    size_t size = atomic_load_explicit(&tab->tab_size, memory_order_relaxed);
    return (size);
}

static struct op_list_item* _get_bucket(struct op_hashtab* tab, uintptr_t key)
{
...
struct op_hashtab* op_hashtab_allocate()
{
    ...
    if (!max_tab_size) {
        max_tab_size =
            op_max(4096, sysconf(_SC_PAGESIZE)) / sizeof(struct op_list_item*);
    }
...
int op_hashtab_purge(struct op_hashtab* tab)
{
    ...
    if ((error = op_list_snapshot(tab->list, (void***)&items, &nb_items))
        != 0) {
        return (error);
    }
    ...
    for (size_t i = 0; i < nb_items; i++) {
        ...
        if (item->value == dummy_value) { continue; }
...
bool op_hashtab_insert(struct op_hashtab* tab, void* key, void* value)
{
    ...
    size_t tab_size =
        atomic_load_explicit(&tab->tab_size, memory_order_relaxed);
