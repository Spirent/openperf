...
const void *icp_dummy_list_value = (void *)0x0ddba11;

/**
 * Internal flags, structures, and defines
 */
enum icp_list_entry_flags {
    LIST_ENTRY_DELETED = (1 << 0),
};

struct icp_list_item;

struct icp_list_entry {
    uint32_t version;            /**< version for avoiding aba problem */
    uint32_t flags;              /**< attributes for this node */
    struct icp_list_item *next;  /**< pointer to list item */
} __attribute__((aligned(16)));

#define ICP_LIST_ENTRY_INIT { 0, 0, NULL }

struct icp_list_item {
    _Atomic struct icp_list_entry entry;
    void *data;
};

/*
 * XXX: Double word alignment is necessary for all
 * struct icp_list_item members
 */
struct icp_list {
    struct icp_list_item head;         /**< list head */
    atomic_size_t length;              /**< # of items in the list */
    icp_comparator *comparator;        /**< list key compare function */
    icp_destructor *destructor;        /**< list item destructor */
    atomic_size_t free_length;         /**< # of items on the freelist */
    struct icp_list_item free_head;    /**< freelist head */
} __attribute__((aligned(64)));

/**
 * Various static functions
 */
static bool _is_deleted(struct icp_list_entry *p)
{
    return (p->flags & LIST_ENTRY_DELETED ? true : false);
}

static void _list_item_destroy(struct icp_list_item **itemp,
                               icp_destructor destructor)
{
    struct icp_list_item *item = *itemp;
...
void _initialize_head_item(struct icp_list_item *head)
{
    head->data = NULL;
    struct icp_list_entry head_entry = ICP_LIST_ENTRY_INIT;
    atomic_init(&head->entry, head_entry);
}

struct icp_list * icp_list_allocate()
{
    struct icp_list *list = NULL;
...
void icp_list_set_comparator(struct icp_list *list, icp_comparator comparator)
{
    list->comparator = comparator;
}

void icp_list_set_destructor(struct icp_list *list, icp_destructor destructor)
{
    list->destructor = destructor;
}

void _destroy_list_items(struct icp_list_item *head,
                         icp_destructor destructor)
{
    struct icp_list_entry head_entry = atomic_load(&head->entry);
    struct icp_list_item *current = head_entry.next;

    while (current != NULL) {
        struct icp_list_entry current_entry = atomic_load(&current->entry);
        struct icp_list_item *next = current_entry.next;
 ...
 * mechanism...
 */
void icp_list_free(struct icp_list **listp)
{
    struct icp_list *list = *listp;
...
int icp_list_purge(struct icp_list *list)
{
    void **items = NULL;
    size_t nb_items  = 0;
    int error = 0;

    if ((error = icp_list_snapshot(list, &items, &nb_items)) != 0) {
        return (error);
    }

    for (size_t i = 0; i < nb_items; i++) {
        void *item = items[i];
        icp_list_delete(list, item);
    }
 ...
 * mechanism...
 */
void icp_list_garbage_collect(struct icp_list *list)
{
    _destroy_list_items(&list->free_head,
                        list->destructor);
...
struct icp_list_item *icp_list_item_allocate(void *value)
{
    assert(value);

    struct icp_list_item *item = calloc(1, sizeof(*item));
    if (!item) {
        return (NULL);
    }
    ...
    struct icp_list_entry item_entry = ICP_LIST_ENTRY_INIT;
    atomic_init(&item->entry, item_entry);

    return (item);
}

void icp_list_item_free(struct icp_list_item **itemp)
...
void *icp_list_item_data(const struct icp_list_item *item)
{
    return (item->data);
}
 ...
 */
bool _freelist_insert(struct icp_list *list, struct icp_list_item *node)
{
    assert(list);
    assert(node);

    struct icp_list_item *head = &list->free_head;
    struct icp_list_entry head_now = atomic_load_explicit(&head->entry,
                                                          memory_order_acquire);
    struct icp_list_entry head_update = ICP_LIST_ENTRY_INIT;

    do {
        /* Update node to point to head's next item */
        struct icp_list_entry node_entry = atomic_load_explicit(&node->entry,
...
int _handle_list_delete(struct icp_list *list,
                        struct icp_list_item *to_delete,
                        struct icp_list_entry *to_delete_entry,
                        struct icp_list_item *prev,
                        struct icp_list_entry *prev_entry)
{
    /*
     * This node has been marked deleted.
     * Update previous to point to next.
     */
    struct icp_list_entry prev_entry_update = {
 ...
 */
bool _icp_list_pfind(struct icp_list *list,
                     struct icp_list_item *start,
                     const void *key,
                     struct icp_list_item **prev,
                     struct icp_list_entry *prev_entry)
{
...
start:
    ...
    *prev_entry = atomic_load_explicit(&(*prev)->entry,
    ...
    while (prev_entry->next != NULL) {
        struct icp_list_item *curr = prev_entry->next;
        struct icp_list_entry curr_entry = atomic_load_explicit(&curr->entry,
                                                                memory_order_acquire);
...
bool icp_list_insert_head_item(struct icp_list *list,
                               struct icp_list_item *item)
{
    return (icp_list_insert_node(list, &list->head, item));
}

bool icp_list_insert_head_value(struct icp_list *list,
                                void *value)
{
    struct icp_list_item *item = icp_list_item_allocate(value);
    if (!item) {
        return (false);
    }

    return (icp_list_insert_head_item(list, item));
}

bool icp_list_insert_node_item(struct icp_list *list,
                               struct icp_list_item *start,
                               struct icp_list_item *item)
{
    assert(list);
    ...
    struct icp_list_item *prev = NULL;
    struct icp_list_entry prev_now = ICP_LIST_ENTRY_INIT;
    struct icp_list_entry prev_update = ICP_LIST_ENTRY_INIT;

    /* Now insert into the list */
    do {
        if (_icp_list_pfind(list, start, item->data, &prev, &prev_now)) {
            return (false);  /* item already in list */
        }

        /* New node goes after previous */
        /* Update node to point to whatever is after previous */
        struct icp_list_entry item_next = {0, 0, prev_now.next};
...
bool icp_list_insert_node_value(struct icp_list *list,
                                struct icp_list_item *start,
                                void *value)
{
    struct icp_list_item *item = icp_list_item_allocate(value);
    ...
    return (icp_list_insert_node_item(list, start, item));
}

void *icp_list_find_head(struct icp_list *list, const void *key)
{
    struct icp_list_entry prev_entry = ICP_LIST_ENTRY_INIT;
    struct icp_list_item *prev = NULL;
    return (_icp_list_pfind(list, &list->head, key, &prev, &prev_entry)
            ? prev_entry.next->data
            : NULL);
}

void *icp_list_find_node(struct icp_list *list,
                         struct icp_list_item *start,
                         const void *key)
{
    struct icp_list_entry prev_entry = ICP_LIST_ENTRY_INIT;
    struct icp_list_item *prev = NULL;
    return (_icp_list_pfind(list, start, key, &prev, &prev_entry)
            ? prev_entry.next->data
            : NULL);
}

bool icp_list_delete_head(struct icp_list *list, const void *key)
{
    return (icp_list_delete_node(list, &list->head, key));
}

bool icp_list_delete_node(struct icp_list *list,
                          struct icp_list_item *start,
                          const void *key)
{
    assert(list);
    ...
    struct icp_list_item *prev = NULL;
    struct icp_list_entry prev_entry = ICP_LIST_ENTRY_INIT;
    struct icp_list_item *to_delete = NULL;
    struct icp_list_entry to_delete_now = ICP_LIST_ENTRY_INIT;
    struct icp_list_entry to_delete_update = ICP_LIST_ENTRY_INIT;

    do {
        if (!_icp_list_pfind(list, start, key, &prev, &prev_entry)) {
...
size_t icp_list_length(struct icp_list *list)
{
    size_t length = atomic_load_explicit(&list->length, memory_order_relaxed);
    return (length);
}

struct icp_list_item *icp_list_head(struct icp_list *list)
{
    return (&list->head);
}

void *icp_list_next(struct icp_list *list, struct icp_list_item **prevp)
{
    (void)list;
    struct icp_list_item *prev = *prevp;
    struct icp_list_entry prev_entry = atomic_load_explicit(&prev->entry,
                                                            memory_order_acquire);
...
int icp_list_snapshot(struct icp_list *list, void **itemsp[], size_t *nb_items)
{
    ...
    do {
        size_t length = icp_list_length(list);
        items = realloc(items, length * sizeof(*items));
        if (!items) {
            return (-ENOMEM);
        }
        struct icp_list_item *cursor = icp_list_head(list);
        void *current = icp_list_next(list, &cursor);
        for (*nb_items = 0;
             *nb_items < length && current != NULL;
             (*nb_items)++, current = icp_list_next(list, &cursor)) {
            items[*nb_items] = current;
        }
        /* If the length has changed, then the list has definitely mutated. */
    } while (*nb_items != icp_list_length(list));
