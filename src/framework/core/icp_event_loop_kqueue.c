...
int _enqueue_new_event(struct icp_event_loop *loop,
                          ...
                          void *arg)
{
    ...
    event->data.type = ICP_EVENT_TYPE_FD;
    event->data.fd = fd;
...
struct icp_event *icp_event_loop_find_fd(struct icp_event_loop *loop, int fd)
{
    ...
    SLIST_FOREACH(event, &loop->events_list, events_link) {
        if (event->data.type == ICP_EVENT_TYPE_FD && event->data.fd == fd) {
            break;
        }
    }
...
#define DELETE_KQ_FILTER_CALLBACK(name, field, filter)                  \
    static int                                                          \
    _delete_kq_##name##_callback(const struct icp_event_data *data,     \
    ...
    for (int i = 0; i < events; i++) {
        if (kerrors[i].flags & EV_ERROR) {
            SLIST_INSERT_IF_MISSING(&loop->update_list,
                                    (struct icp_event *)kerrors[i].udata,
                                    update_link);
        }
    }
    return (0);
...
size_t _do_event_updates(struct icp_event_loop *loop)
{
    size_t idx = 0;
    struct kevent kevents[loop->nb_events * 2];
    uint16_t base_flags = (loop->flags & ICP_EVENT_LOOP_EDGETRIGGER) ? EV_CLEAR : 0;
    ...
    if (idx && kevent(loop->poll_fd, kevents, idx, NULL, 0, NULL) == -1) {
        icp_exit("Failed to add events to kqueue %d: %s\n",
                       ...
                       struct kevent *kevents, size_t nb_events)
{
    for (size_t i = 0; i < nb_events; i++) {
        ...
        struct icp_event *event = (struct icp_event *)kev->udata;

        switch (kev->filter) {
        case EVFILT_READ:
            if (event->data.type == ICP_EVENT_TYPE_ZMQ
                && !icp_event_loop_get_socket_is_readable(event->data.socket)) {
        ...
        case EVFILT_WRITE:
            if (event->data.type == ICP_EVENT_TYPE_ZMQ
                && !icp_event_loop_get_socket_is_writable(event->data.socket)) {
                continue;  /* false alarm */
            }
...
size_t _do_event_removals(struct icp_event_loop *loop)
{
    size_t idx = 0;
    struct kevent kevents[loop->nb_events * 2];
