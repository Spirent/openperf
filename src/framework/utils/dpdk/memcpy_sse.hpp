...
#ifndef _OP_UTILS_DPDK_MEMCPY_SSE_HPP_
#define _OP_UTILS_DPDK_MEMCPY_SSE_HPP_
...
inline constexpr int alignment_mask = 0x0f;
...
inline void* rte_memcpy_generic(void* dst, const void* src, size_t n)
{
    ...
    if (n <= 128) { goto COPY_BLOCK_128_BACK15; }
    if (n <= 512) {
        if (n >= 256) {
            n -= 256;
...
#endif /* _OP_UTILS_DPDK_MEMCPY_SSE_HPP_ */
