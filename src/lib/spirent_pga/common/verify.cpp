...
uint32_t prbs(const uint8_t payload[], uint16_t length)
{
    ...
    /*
     * We have no idea what the seed for the incoming PRBS data is, so we
     * presume that the payload is at least partially correct and attempt to
     * recover the seed from it.  This means that we can't actually check the
     * first 32 bits of data for correctness.  However, we might be able to use
    ...
    /*
     * Use the seed returned by the verify function to check any remaining data
     * that was not checked by the aligned function.
     */
