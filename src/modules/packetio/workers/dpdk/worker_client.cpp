...
{
    if (auto error =
            send_message(m_socket.get(), add_descriptors_msg{descriptors});
        error && error != ETERM) {
        throw std::runtime_error(
            "Could not send add descriptors message to workers: "
            + std::string(zmq_strerror(error)));
    }
}
...
{
    if (auto error =
            send_message(m_socket.get(), del_descriptors_msg{descriptors});
        error && error != ETERM) {
        throw std::runtime_error(
            "Could not send delete descriptors message to workers: "
            + std::string(zmq_strerror(error)));
    }
}
