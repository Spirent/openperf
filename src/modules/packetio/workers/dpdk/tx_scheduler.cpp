...
void tx_scheduler::do_reschedule(const schedule::time_point& now)
{
    ...
    m_time_reschedule += schedule_poll;  /* Update from previous value to prevent drift */
}
...
std::optional<schedule::state> tx_scheduler::on_timeout(const schedule::state_blocked& blocked)
{
    ...
    auto source = m_tib.get_source(key);
    if (!source) return (schedule::state_running{});
     ...
     * Resume normal operation by jumping into the run handler.  If the
     * running state handler doesn't return a new state, then we are
     * still in the run state.  Let the upper layer know by returning
     * that state explicitly.
     * Also, use the current time as the next source deadline so that we
     * don't generate new events in the past.
     */
