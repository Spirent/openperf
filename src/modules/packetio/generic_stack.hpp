#ifndef _OP_PACKETIO_GENERIC_STACK_HPP_
#define _OP_PACKETIO_GENERIC_STACK_HPP_
...
private:
    struct stack_concept
    {
        ...
        virtual std::optional<interface::generic_interface>
        interface(std::string_view id) const = 0;
        virtual tl::expected<std::string, std::string>
        create_interface(const interface::config_data& config) = 0;
    ...
    template <typename Stack> struct stack_model final : stack_concept
    {
        ...
        tl::expected<std::string, std::string>
        create_interface(const interface::config_data& config) override
        {
