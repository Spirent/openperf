...
queueing::queueing(std::string_view if_prefix,
                   uint16_t if_idx,
                   uint16_t port_idx)
    : m_notify(false)
{
    ...
    m_queue = std::unique_ptr<rte_ring, rte_ring_deleter>(
        (rte_ring_create(recvq_name(if_prefix, if_idx).c_str(),
                         rx_queue_size,
                         rte_eth_dev_socket_id(port_idx),
                         RING_F_SC_DEQ)));
}
...
err_t queueing::handle_rx_notify(netif* netintf)
{
    ...
    do {
        auto nb_pbufs =
            rte_ring_dequeue_burst(m_queue.get(),
...
err_t queueing::handle_rx(pbuf* p, netif* netintf)
{
    if (rte_ring_enqueue(m_queue.get(), p) != 0) {
        OP_LOG(OP_LOG_WARNING,
               "Receive queue on %c%c%u is full; packet dropped!\n",
               netintf->name[0],
               netintf->name[1],
               netintf->num);
        return (ERR_BUF);
...
err_t direct::handle_rx_notify(netif* netintf)
{
    ...
    OP_LOG(OP_LOG_ERROR,
           "Calling useless rx notify function on %c%c%u\n",
           netintf->name[0],
           netintf->name[1],
           netintf->num);
    return (ERR_IF);
