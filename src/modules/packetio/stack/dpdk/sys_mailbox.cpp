...
sys_mbox::~sys_mbox() { close(m_fd); }

int sys_mbox::fd() const { return (m_fd); }
...
static void do_read(int fd)
{
    ...
    if (error)
        OP_LOG(OP_LOG_ERROR, "Could not read fd %d: %s\n", fd, strerror(errno));
}
...
{
    if (m_armed.test_and_set(std::memory_order_acq_rel)) { do_read(m_fd); }
...
void sys_mbox::post(void* msg)
{
    ...
    while (rte_ring_enqueue(m_ring.get(), msg) == -ENOBUFS) { rte_pause(); }
...
{
    void* msg = nullptr;
    return (rte_ring_dequeue(m_ring.get(), &msg) == 0 ? msg : nullptr);
}
