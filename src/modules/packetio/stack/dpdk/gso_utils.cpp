...
{
    /*
     * We need to query the outgoing interface to see if it supports GSO
     * and get the maximum segment size.  If the socket is bound to the
     * interface, then we can look up the interface directly; otherwise
     * we have to do a route lookup.
     */
    ...
    auto length = (info.tx_offloads() & DEV_TX_OFFLOAD_TCP_TSO
                   ? std::max(static_cast<int>(std::numeric_limits<uint16_t>::max()),
                              info.tx_tso_segment_max() * PBUF_POOL_BUFSIZE)
                   : TCP_MSS);
    return (length);
}
