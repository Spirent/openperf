...
{
    static const auto ptype = RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_UDP;
    return ((mbuf->packet_type & ptype) == ptype
                                  ...
                                  void* user_param)
{
    ...
    /*
     * Given the epoch offset, find the offset that will shift the current
     * wall clock time to the 38 bit, 2.5 ns timestamp field, in nanoseconds.
     * The epoch_offset should be the number of seconds sinece the beginning
     * of the current year.
     */
    ...
    const auto now = utils::get_phxtime_offset<clock>(epoch_offset);
