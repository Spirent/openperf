 ...
 */
static void log_flow_error(enum flow_error_type type,
                  ...
                  rule_action action)
{
    ...
    if (!flow) {
        ...
        return (
            tl::make_unexpected(rte_errno)); /* What else could we return? */
    }
...
{
    if (add.on_overflow) { add.on_overflow.value()(); }
}

static void maybe_invoke_callback(const filter_event_del& del)
{
    if (del.on_underflow) { del.on_underflow.value()(); }
}
...
uint16_t flow_filter::port_id() const { return (m_port); }
                                                  ...
                                                  const filter_state_ok&)
{
    ...
    if (!flow) { return (filter_state_error{}); }
                                                  ...
                                                  const filter_state_ok&)
{
    ...
    if (!delete_flow(m_port, item->second)) { return (filter_state_error{}); }
...
std::optional<filter_state> flow_filter::on_event(const filter_event_enable&,
                                                  const filter_state_disabled&)
{
    if (!maybe_disable_promiscuous_mode(m_port, m_flows)) {
        return (filter_state_error{});
