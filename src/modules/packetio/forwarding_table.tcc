...
template <typename Interface, typename Sink, int MaxPorts>
bool
forwarding_table<Interface, Sink, MaxPorts>::has_interface_sinks(uint16_t port_idx) const
{
    assert(port_idx < MaxPorts);
    auto count = m_port_interface_sink_count[port_idx].load(std::memory_order_consume);
    return (count > 0);
}

template <typename Interface, typename Sink, int MaxPorts>
const std::vector<Sink>*
forwarding_table<Interface, Sink, MaxPorts>::find_interface_sinks(uint16_t port_idx, const net::mac_address& mac) const
{
    assert(port_idx < MaxPorts);
