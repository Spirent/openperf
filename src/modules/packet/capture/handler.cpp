...
public:
    ...
    void get_captures_pcap(const request_type& request, response_type response);
    ...
    : m_socket(op_socket_get_client(context, ZMQ_REQ, endpoint.data()))
{
    ...
    Post(router,
         "/packet/captures/x/pcap",
         bind(&handler::get_captures_pcap, this));
}
...
void handler::get_captures_pcap(const request_type& request,
                                response_type response)
{
    auto swagger_request =
