...
bool server::has_active_transfer(const sink& sink) const
{
    ...
    if (auto found = std::find_if(m_results.begin(),
                                  m_results.end(),
                                  [&](const auto& pair) {
    ...
    if (trash_map::const_iterator found = m_trash.find(sink.id());
        found != m_trash.end()) {
        auto& bucket = found->second;
        auto& results = bucket->results;
        std::vector<result_value_ptr>::const_iterator found_result =
...
void server::add_trash(sink_value_type& item)
{
    ...
    OP_LOG(OP_LOG_INFO, "Added capture %s to trash.", sink_ref.id().c_str());
    bucket->sink = item;
    move_if(
        m_results,
        bucket->results,
...
{
    OP_LOG(OP_LOG_INFO, "Starting capture garbage collection");
    ...
    OP_LOG(OP_LOG_INFO, "Completed capture garbage collection");
    return -1;
}
