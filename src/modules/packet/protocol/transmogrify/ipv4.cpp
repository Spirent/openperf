...
std::shared_ptr<swagger::v1::model::PacketProtocolIpv4> to_swagger(packet::protocol::ipv4& src)
{
    ...
    case ipv4::ecn_value::non_ect:
        dst->setEcn("non_ect");
        break;
    case ipv4::ecn_value::ect_0:
        dst->setEcn("ect_0");
        break;
    case ipv4::ecn_value::ect_1:
        dst->setEcn("ect_1");
        break;
    case ipv4::ecn_value::ce:
        dst->setEcn("ce");
        break;
    }
    ...
    if (src_flags & ipv4::flags_value::evil_bit)
    {
        dst->getFlags().emplace_back("evil_bit");
    }
    if (src_flags & ipv4::flags_value::dont_fragment)
    {
        dst->getFlags().emplace_back("dont_fragment");
    }
    if (src_flags & ipv4::flags_value::more_fragments)
    {
...
static enum ipv4::ecn_value to_ipv4_ecn(std::string_view value)
{
    if (value == "non_ect")
    {
        return (ipv4::ecn_value::non_ect);
    }
    else if (value == "ect_0")
    {
        return (ipv4::ecn_value::ect_0);
    }
    else if (value == "ect_1")
    {
        return (ipv4::ecn_value::ect_1);
    }
    else if (value == "ce")
    {
        return (ipv4::ecn_value::ce);
    }

    return (static_cast<ipv4::ecn_value>(0));
}

static type::bit_flags<ipv4::flags_value> to_ipv4_flags(std::vector<std::string>& values)
{
    auto tmp = type::bit_flags<ipv4::flags_value>{0};
    for (auto& value : values)
    {
        if (value == "evil_bit")
        {
            tmp |= ipv4::flags_value::evil_bit;
        }
        else if (value == "dont_fragment")
        {
            tmp |= ipv4::flags_value::dont_fragment;
        }
        else if (value == "more_fragments")
        {
            tmp |= ipv4::flags_value::more_fragments;
        }
    }
    return (tmp);
}

packet::protocol::ipv4 to_protocol(std::shared_ptr<swagger::v1::model::PacketProtocolIpv4>& src)
{
    auto dst = packet::protocol::ipv4{};
    ...
    if (src->sourceIsSet())
    {
        set_ipv4_source(dst, packet::type::ipv4_address(src->getSource()));
    }

    if (src->destinationIsSet())
    {
        set_ipv4_destination(dst, packet::type::ipv4_address(src->getDestination()));
    }
