#ifndef _OP_PACKET_PROTOCOL_STATISTICS_PACKET_TYPE_COUNTERS_HPP_
#define _OP_PACKET_PROTOCOL_STATISTICS_PACKET_TYPE_COUNTERS_HPP_
...
struct transport final : packet_type_counters<8>
{
    ...
    transport() = default;
    transport(const packet_type_counters<8>& base)
...
struct inner_transport final : packet_type_counters<8>
{
    ...
    inner_transport() = default;
    inner_transport(const packet_type_counters<8>& base)
        : packet_type_counters<8>(base)
    {}
};
...
void update(StatsTuple& tuple,
            const packetio::packet::packet_type::flags fields[],
            uint16_t nb_fields)
{
    /*
    ...
    if constexpr (std::tuple_size_v<StatsTuple> != 0) {
        ...
        pga_unpack_and_sum_indexicals(reinterpret_cast<const uint32_t*>(fields),
                                      nb_fields,
                                      masks.data(),
                                      masks.size(),
                                      counters.data());
...
void dump(std::ostream& os, const StatsTuple& tuple)
{
    ...
    if constexpr (has_type<transport, StatsTuple>::value) {
        dump(os, get_counter<transport, StatsTuple>(tuple));
    }
    ...
    if constexpr (has_type<inner_transport, StatsTuple>::value) {
        dump(os, get_counter<inner_transport, StatsTuple>(tuple));
    }
}

} // namespace openperf::packet::statistics::protocol

#endif /* _OP_PACKET_PROTOCOL_COUNTERS_PACKET_TYPE_COUNTERS_HPP_ */
