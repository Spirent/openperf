 ...
 * @file
 *
 * The content of some of these functions are based on the lwip/GSO
 * implementation contained here: https://github.com/sysml/lwip.git
 */
...
namespace openperf::packet::stack::dpdk::gso {
...
static std::tuple<pbuf*, uint16_t>
pbuf_drop_at(struct pbuf* p_head, uint16_t offset, uint16_t length)
{
    ...
    for (auto p = p_head; p != start; p = p->next) { p->tot_len -= length; }
...
pbuf_split_at(struct pbuf* p_head, uint16_t split, pbuf_layer layer)
{
    ...
    struct pbuf* split_pbuf =
        pbuf_skip_with_previous(p_head, split, &split_offset, &split_prev);
    ...
    if (split_offset == 0) {
        ...
        LWIP_ASSERT("split pbuf header too small",
                    layer <= packet_stack_pbuf_header_available(split_pbuf));
        ...
        LWIP_ASSERT("split pbuf header too small",
                    layer <= packet_stack_pbuf_header_available(next_pbuf));
...
using namespace openperf::packet::stack::dpdk;

uint16_t packet_stack_gso_oversize_calc_length(uint16_t length)
{
    /*
    ...
    static constexpr uint16_t payload_size =
        PBUF_POOL_BUFSIZE - RTE_PKTMBUF_HEADROOM;
    static_assert(payload_size != 0
                  && (payload_size & (payload_size - 1)) == 0);
...
{
    return (
        ((std::numeric_limits<uint16_t>::max() - PBUF_POOL_BUFSIZE) / TCP_MSS)
        * TCP_MSS);
}

uint16_t packet_stack_gso_max_segment_length(const struct tcp_pcb* pcb)
{
    /*
...
int packet_stack_gso_segment_split(struct tcp_pcb* pcb,
                                   struct tcp_seg* seg,
                                   uint16_t split)
{
    LWIP_ERROR("pcb != NULL && seg != NULL && split != 0 && split < seg->len "
               "(programmer violates API)",
               ((pcb != nullptr) && (seg != nullptr) && (split != 0)
                && (split < seg->len)),
               return ERR_ARG;);
    ...
    /*
     * Split the pbufs into two chains. One remains attached to the segment, the
     * other is returned by this function.
     */
    auto [next_pbuf, pbuf_incr] =
        gso::pbuf_split_at(seg->p, seg_header_length + split, PBUF_TRANSPORT);
    if (!next_pbuf) {
        LWIP_DEBUGF(TCP_OUTPUT_DEBUG | LWIP_DBG_LEVEL_SERIOUS,
    ...
    if (next_seg->next == nullptr) {
        ...
        pcb->unsent_oversize =
            LWIP_MIN(packet_stack_gso_pbuf_data_available(p, p->len), TCP_MSS);
#if TCP_OVERSIZE_DBGCHECK
    ...
    next_seg->tcphdr->seqno =
        lwip_htonl(lwip_ntohl(seg->tcphdr->seqno) + seg->len);
...
uint16_t packet_stack_gso_pbuf_data_available(const struct pbuf* p,
                                              uint16_t offset)
{
    auto max_payload = packet_stack_pbuf_data_available(p);
    return (max_payload > offset ? max_payload - offset : 0);
}

uint16_t packet_stack_gso_pbuf_copy(struct pbuf* p_head,
                                    uint16_t offset,
                                    const void* dataptr,
                                    uint16_t length)
{
    ...
    for (auto p = p_head; length != 0 && p != nullptr; p = p->next) {
        auto available = packet_stack_gso_pbuf_data_available(p, offset);
        if (!available) {
            offset -= p->len;
        } else {
