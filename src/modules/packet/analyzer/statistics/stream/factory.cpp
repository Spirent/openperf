...
constexpr auto to_value(openperf::utils::bit_flags<stream_flags> flags)
{
    return (flags.value);
}
...
template <int FlagValue> auto make_counter_tuple()
{
    ...
    auto t7 = maybe_tuple_cat<static_cast<bool>(
        FlagValue & to_value(stream_flags::jitter_ipdv))>(
...
{
    return (
        []() { return (generic_stream_counters(make_counter_tuple<I>())); });
}

template <size_t... I>
auto make_counters_constructor_index(std::index_sequence<I...>)
{
    auto m = std::vector<std::function<generic_stream_counters()>>{};
