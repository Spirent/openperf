...
template <typename T> inline bintime operator*(const bintime& lhs, T rhs)
{
    ...
    return (
        bintime{.bt_sec = static_cast<sec_t>((lhs.bt_sec * rhs) + (tmp2 >> 32)),
...
{
    static_assert(std::is_integral<T>::value);
    if (rhs == 0) { throw std::domain_error("cannot divide by 0"); }

    auto tmp =
        bintime{.bt_sec = lhs.bt_sec / rhs, .bt_frac = lhs.bt_frac / rhs};
    auto remainder =
        bintime{.bt_frac = (((1ULL << 63) / rhs) * (lhs.bt_sec % rhs)) << 1};
