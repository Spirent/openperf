...
const char* to_string(const udp_socket_state& state)
{
    return (std::visit(
        utils::overloaded_visitor(
            [](const udp_init&) -> const char* { return ("init"); },
            [](const udp_bound&) -> const char* { return ("bound"); },
            [](const udp_connected&) -> const char* { return ("connected"); },
            [](const udp_closed&) -> const char* { return ("closed"); }),
        state));
}
    ...
    void* arg, udp_pcb* pcb, pbuf* p, const ip_addr_t* addr, in_port_t port)
{
    ...
    /*
     * Note: we switch the byte order of the port here because lwip provides the
...
channel_variant udp_socket::channel() const { return (m_channel.get()); }
...
static tl::expected<void, int> do_udp_bind(udp_pcb* pcb,
    ...
    udp_pcb* pcb, const api::request_connect& connect, dgram_channel* channel)
{
    ...
    auto error =
        (IP_IS_ANY_TYPE_VAL(*ip_addr) ? do_udp_disconnect(pcb)
...
udp_socket::do_getsockopt(const udp_pcb* pcb,
...
udp_socket::do_setsockopt(udp_pcb* pcb,
...
udp_socket::on_request_reply
udp_socket::on_request(const api::request_bind& bind, const udp_init&)
{
...
udp_socket::on_request_reply
udp_socket::on_request(const api::request_connect& connect, const udp_init&)
{
...
udp_socket::on_request_reply
udp_socket::on_request(const api::request_connect& connect, const udp_bound&)
{
    auto result = do_udp_connect(m_pcb.get(), connect, m_channel.get());
    if (!result) return {tl::make_unexpected(result.error()), std::nullopt};
    if (!*result)
...
udp_socket::on_request_reply
udp_socket::on_request(const api::request_connect& connect,
                       const udp_connected&)
{
    auto result = do_udp_connect(m_pcb.get(), connect, m_channel.get());
    if (!result) return {tl::make_unexpected(result.error()), std::nullopt};
...
udp_socket::on_request_reply
udp_socket::on_request(const api::request_getpeername& getpeername,
                       const udp_connected&)
{
    auto result =
        do_udp_get_name(m_pcb->remote_ip, m_pcb->remote_port, getpeername);
    if (!result) return {tl::make_unexpected(result.error()), std::nullopt};
    return {api::reply_socklen{*result}, std::nullopt};
}

udp_socket::on_request_reply
udp_socket::on_request(const api::request_getsockname& getsockname,
                       const udp_bound&)
{
    auto result =
        do_udp_get_name(m_pcb->local_ip, m_pcb->local_port, getsockname);
    if (!result) return {tl::make_unexpected(result.error()), std::nullopt};
    return {api::reply_socklen{*result}, std::nullopt};
}
