...
extern "C" int packetio_memory_pbuf_timestamp_offset(const struct pbuf *p);

uint8_t raw_receive(void* arg, raw_pcb* pcb, pbuf* p, const ip_addr_t* addr)
{
    (void)pcb;
    assert(addr != nullptr);
    auto arg_ = reinterpret_cast<receive_arg*>(arg);
    auto icmp_filter = arg_->rsock->icmp_filter();
    auto iphdr = (ip_hdr *)p->payload;
    ...
    if (arg_->rsock->option(SO_TIMESTAMP)) {
        struct timeval tv;
        gettimeofday(&tv, NULL);

        auto tstamp = reinterpret_cast<uint64_t*>(reinterpret_cast<uintptr_t>(p) -
                packetio_memory_pbuf_timestamp_offset(p));
        *tstamp = tv.tv_sec << 32 | tv.tv_usec;
        std::cerr << "AZ: tstamp initialized to " << *tstamp << ", " << tstamp << std::endl;
        if (!arg_->dchan->send(p, reinterpret_cast<const dgram_ip_addr*>(addr), 0, tstamp)) {
            return (0);
        }
    } else {
        if (!arg_->dchan->send(p, reinterpret_cast<const dgram_ip_addr*>(addr), 0)) {
            return (0);
        }
    }
    return (1);
...
raw_socket::raw_socket(icp::socket::server::allocator& allocator, int flags)
    : m_channel(new (allocator.allocate(sizeof(dgram_channel)))
                dgram_channel(flags), dgram_channel_deleter(&allocator))
    , m_pcb(raw_new(IP_PROTO_ICMP))
{
    if (!m_pcb) {
        throw std::runtime_error("Out of RAW pcb's!");
    }

    static receive_arg arg = {
        .dchan = m_channel.get(),
        .rsock = this
    };
    raw_recv(m_pcb.get(), &raw_receive, &arg);
    ...
    case SOL_SOCKET:
        ...
        case SO_TIMESTAMP: {
            ...
            if (*opt) {
                ...
                std::cerr << "AZ: SO_TIMESTAMP on" << std::endl;
            } else {
                m_options &= ~(1 << SO_TIMESTAMP);
            }
            return {};
