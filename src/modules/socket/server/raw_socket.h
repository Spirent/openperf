...
public:
    raw_socket(icp::socket::server::allocator& allocator, int flags);
    ~raw_socket() = default;

    raw_socket(const raw_socket&) = delete;
    raw_socket& operator=(const raw_socket&&) = delete;

    raw_socket& operator=(raw_socket&& other) = default;
    raw_socket(raw_socket&& other) = default;
    ...
    {
        if (m_options & (1 << opt)) return true;
        return false;
    }
