...
static constexpr std::string_view yama_ptrace_file =
                     ...
                     size_t nb_cap_values)
{
    ...
    auto cursor = std::partition(
        cap_values, cap_values + nb_cap_values, [&](auto& cap_value) {
            cap_flag_value_t flag = CAP_CLEAR;
            if (auto error =
                    cap_get_flag(caps.get(), cap_value, CAP_EFFECTIVE, &flag);
                error != 0) {
                fprintf(
                    output,
    ...
    return ((nb_unset_flags == 0 || cap_set_proc(caps.get()) == 0) ? true
                                                                   : false);
}
...
bool enable_ptrace(FILE* output, std::optional<pid_t> pid)
{
    ...
    /* Processes must be "dumpable" in order for any ptrace attachment to
     * succeed */
    ...
    case 2: {
        ...
        if (!get_caps(output, required_caps.data(), required_caps.size())) {
            fprintf(
                output,
                "OpenPerf and its clients need the CAP_SYS_PTRACE capability. "
                "Alternatively, consider changing the value of %.*s to 1.\n",
                static_cast<int>(yama_ptrace_file.size()),
                yama_ptrace_file.data());
            result = false;
        }
        break;
    }
    default:
        ...
        fprintf(output,
                "Ptrace functionality is disabled. "
