...
struct task_operation_stat_t {
    ...
    uint_fast64_t bytes_target; /* The intended number of bytes read or written */
    uint_fast64_t bytes_actual; /* The actual number of bytes read or written */
    uint_fast64_t errors;       /* The number of io_errors observed during reading or writing */
    uint_fast64_t latency;      /* The total amount of time required to perform all operations */
...
private:
    ...
    task_stat_t stat1, stat2; // Switchable actual statistics
    task_stat_t* task_stat; // Statistics in work
    ...
    size_t worker_spin(int (*queue_aio_op)(aiocb *aiocb), task_operation_stat_t& op_stat);
