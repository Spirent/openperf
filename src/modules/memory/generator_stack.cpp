...
tl::expected<MemoryGeneratorPointer, std::string> generator_stack::create(const MemoryGenerator& generator)
{
    if (contains(generator.getId()))
        return tl::make_unexpected("Memory generator with id '"
            + generator.getId() + "' already exists.");

    auto ptr = MemoryGeneratorPointer(
        new MemoryGenerator(generator));
