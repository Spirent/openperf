...
int coordinator::do_load_update()
{
    ...
    if (auto last = sum.last()) {
        ...
        auto read_rate = static_cast<double>(sum.read.ops_actual
                                             - m_prev_sum->read.ops_actual)
                         / to_seconds(delta_t).count();
        auto write_rate = static_cast<double>(sum.write.ops_actual
                                              - m_prev_sum->write.ops_actual)
                          / to_seconds(delta_t).count();
