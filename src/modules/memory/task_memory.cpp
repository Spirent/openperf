...
task_memory::task_memory(const task_memory_config& conf)
    : m_scratch{buffer::STDLIB}
    , m_pid(0.01, 0.0005, 0.0)
{
    auto cache_line_size =
        static_cast<size_t>(sysconf(_SC_LEVEL1_DCACHE_LINESIZE));
    if (!cache_line_size) m_scratch = buffer{cache_line_size, buffer::STDLIB};
    config(conf);
}
...
void task_memory::config(const task_memory_config& msg)
{
    ...
    if ((m_config.block_size = msg.block_size) > m_scratch.size()) {
        ...
        assert(m_scratch.resize(msg.block_size));
