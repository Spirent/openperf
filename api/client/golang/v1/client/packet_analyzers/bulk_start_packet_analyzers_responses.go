// Code generated by go-swagger; DO NOT EDIT.

package packet_analyzers

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"fmt"
	"io"
	"strconv"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"

	"github.com/spirent/openperf/api/client/golang/v1/models"
)

// BulkStartPacketAnalyzersReader is a Reader for the BulkStartPacketAnalyzers structure.
type BulkStartPacketAnalyzersReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *BulkStartPacketAnalyzersReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewBulkStartPacketAnalyzersOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewBulkStartPacketAnalyzersOK creates a BulkStartPacketAnalyzersOK with default headers values
func NewBulkStartPacketAnalyzersOK() *BulkStartPacketAnalyzersOK {
	return &BulkStartPacketAnalyzersOK{}
}

/* BulkStartPacketAnalyzersOK describes a response with status code 200, with default header values.

Success
*/
type BulkStartPacketAnalyzersOK struct {
	Payload *BulkStartPacketAnalyzersOKBody
}

func (o *BulkStartPacketAnalyzersOK) Error() string {
	return fmt.Sprintf("[POST /packet/analyzers/x/bulk-start][%d] bulkStartPacketAnalyzersOK  %+v", 200, o.Payload)
}
func (o *BulkStartPacketAnalyzersOK) GetPayload() *BulkStartPacketAnalyzersOKBody {
	return o.Payload
}

func (o *BulkStartPacketAnalyzersOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(BulkStartPacketAnalyzersOKBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

/*BulkStartPacketAnalyzersBody BulkStartPacketAnalyzersRequest
//
// Parameters for the bulk start operation
swagger:model BulkStartPacketAnalyzersBody
*/
type BulkStartPacketAnalyzersBody struct {

	// List of analyzer identifiers
	// Required: true
	// Min Items: 1
	Ids []string `json:"ids"`
}

// Validate validates this bulk start packet analyzers body
func (o *BulkStartPacketAnalyzersBody) Validate(formats strfmt.Registry) error {
	var res []error

	if err := o.validateIds(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (o *BulkStartPacketAnalyzersBody) validateIds(formats strfmt.Registry) error {

	if err := validate.Required("start"+"."+"ids", "body", o.Ids); err != nil {
		return err
	}

	iIdsSize := int64(len(o.Ids))

	if err := validate.MinItems("start"+"."+"ids", "body", iIdsSize, 1); err != nil {
		return err
	}

	return nil
}

// ContextValidate validates this bulk start packet analyzers body based on context it is used
func (o *BulkStartPacketAnalyzersBody) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (o *BulkStartPacketAnalyzersBody) MarshalBinary() ([]byte, error) {
	if o == nil {
		return nil, nil
	}
	return swag.WriteJSON(o)
}

// UnmarshalBinary interface implementation
func (o *BulkStartPacketAnalyzersBody) UnmarshalBinary(b []byte) error {
	var res BulkStartPacketAnalyzersBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*o = res
	return nil
}

/*BulkStartPacketAnalyzersOKBody BulkStartPacketAnalyzersResponse
swagger:model BulkStartPacketAnalyzersOKBody
*/
type BulkStartPacketAnalyzersOKBody struct {

	// List of analyzer results
	// Required: true
	// Min Items: 1
	Items []*models.PacketAnalyzerResult `json:"items"`
}

// Validate validates this bulk start packet analyzers o k body
func (o *BulkStartPacketAnalyzersOKBody) Validate(formats strfmt.Registry) error {
	var res []error

	if err := o.validateItems(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (o *BulkStartPacketAnalyzersOKBody) validateItems(formats strfmt.Registry) error {

	if err := validate.Required("bulkStartPacketAnalyzersOK"+"."+"items", "body", o.Items); err != nil {
		return err
	}

	iItemsSize := int64(len(o.Items))

	if err := validate.MinItems("bulkStartPacketAnalyzersOK"+"."+"items", "body", iItemsSize, 1); err != nil {
		return err
	}

	for i := 0; i < len(o.Items); i++ {
		if swag.IsZero(o.Items[i]) { // not required
			continue
		}

		if o.Items[i] != nil {
			if err := o.Items[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("bulkStartPacketAnalyzersOK" + "." + "items" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// ContextValidate validate this bulk start packet analyzers o k body based on the context it is used
func (o *BulkStartPacketAnalyzersOKBody) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := o.contextValidateItems(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (o *BulkStartPacketAnalyzersOKBody) contextValidateItems(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(o.Items); i++ {

		if o.Items[i] != nil {
			if err := o.Items[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("bulkStartPacketAnalyzersOK" + "." + "items" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (o *BulkStartPacketAnalyzersOKBody) MarshalBinary() ([]byte, error) {
	if o == nil {
		return nil, nil
	}
	return swag.WriteJSON(o)
}

// UnmarshalBinary interface implementation
func (o *BulkStartPacketAnalyzersOKBody) UnmarshalBinary(b []byte) error {
	var res BulkStartPacketAnalyzersOKBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*o = res
	return nil
}
