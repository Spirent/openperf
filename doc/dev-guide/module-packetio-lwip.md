...
```
In this second `pbuf_realloc` function,  the code shriking the _pbuf_ payload is removed. That's because the encapsulating DPDK _mbuf_ copes with payload headroom and tailroom, so it does not make sense to shrink the payload. 